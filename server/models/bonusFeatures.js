        ...
        async function recursivePromiseFunction(featuresForMovie, movie) {
          try {
            return await new Promise(async (resolve, reject) => {
              ffmpeg.ffprobe(
                `G:/bonusFeatures/${movie}/${featuresForMovie[i]}`,
                async function (err, metaData) {
                  const id = execSync(
                    `mkvmerge -i "G:/bonusFeatures/${movie}/${featuresForMovie[i]}"`
                  )
                    .toString()
                    .split("\n")
                  ...
                  await execSync(
                    `mkvextract "G:/bonusFeatures/${movie}/${
                      featuresForMovie[i]
                    }" attachments "${id}:G:/bonusFeatures/thumbnails/${featuresForMovie[
                      i
                    ].replace("mkv", "")}.jpg"`
                  );
