...
async function main(video) {
  try {
    await execute(
      `I:/ffmpeg -y -i "I:/Videos/${video}.mkv" -map 0:0 -c copy I:/BL_EL_RPU.hevc -map 0:1 -c eac3 -ac 6 I:/pcm.mov`
    );
    await execute(`I:/dovi_tool -m 2 convert I:/BL_EL_RPU.hevc --discard`);
    await execute(
      `I:/mp4box -tmp "I:/" -add BL_RPU.hevc:dvp=5 -add I:/pcm.mov:lang=eng -new "I:/Videos/${video}.mp4"`
    );
    await execute(
      `move "I:\\Videos\\${video}.mkv" "I:\\ConvertedDVMKVs\\${video}.mkv"`
    );
